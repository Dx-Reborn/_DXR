...
// Для отладки ИИ, скопировано из UnrealPlayer
...
// Возможность использовать что-то ещё.
// Стандартная функция Unreal Tournament 2004
// Используется в т.ч. для посадки/выхода в/из транспорта.
/*function UseActor()
{
  local Actor A;
    ...
    ForEach Pawn.VisibleCollidingActors(class'Mover', mv, MaxFrobDistance)
    {
			mv.usedBy(pawn);
            return;
    }

    ForEach Pawn.VisibleCollidingActors(class'Decoration', deco, MaxFrobDistance)
    {
			deco.usedBy(pawn);
            return;
    }
...
{
	clientmessage("PostLoadSavedGame()");
	log("PostLoadSavedGame()");
}

event PreSaveGame()
{
	clientmessage("PreSaveGame()");
	Log("PreSaveGame()");
}
...
event TravelPostAccept()
{
  super.TravelPostAccept();
	DeusExGameInfo(Level.Game).PostTravel(DeusExPlayer(Pawn));
...
//====================================================
// Пока автоподбор не удален, функция почти бесполезна
// 31/07/2017 : Touch заменен на Frob, как в оригинале.
exec function SpawnAllAmmo()
{
	local Vector loc;
	...
	loc = pawn.Location + 2 * pawn.CollisionRadius * Vector(GetViewRotation());

	Spawn(class'AmmoShuriken',,, loc);
	Spawn(class'AmmoShell',,, loc);
	Spawn(class'AmmoSabot',,, loc);
	Spawn(class'AmmoRocketWP',,, loc);
	Spawn(class'AmmoRocketRobot',,, loc);
	Spawn(class'AmmoRocketMini',,, loc);
	Spawn(class'AmmoRocket',,, loc);
	Spawn(class'AmmoPlasma',,, loc);
	Spawn(class'AmmoPepper',,, loc);
	Spawn(class'AmmoNone',,, loc);
	Spawn(class'AmmoNapalm',,, loc);
	Spawn(class'AmmoNanoVirusGrenade',,, loc);
	Spawn(class'AmmoLAM',,, loc);
	Spawn(class'AmmoGreaselSpit',,, loc);
	Spawn(class'AmmoGraySpit',,, loc);
	Spawn(class'AmmoGasGrenade',,, loc);
	Spawn(class'AmmoEMPGrenade',,, loc);
	Spawn(class'AmmoDart',,, loc);
	Spawn(class'AmmoBattery',,, loc);
	Spawn(class'Ammo762mm',,, loc);
	Spawn(class'Ammo3006',,, loc);
	Spawn(class'Ammo20mm',,, loc);
	Spawn(class'Ammo10mm',,, loc);
	Spawn(class'AmmoDartFlare',,, loc);
	Spawn(class'AmmoDartPoison',,, loc);
...
function StreamInteraction GetSI()
{
	local int i;
	local StreamInteraction s;
	...
	if ( Player != None )
	{
		for ( i = 0; i < Player.LocalInteractions.Length; i++ )
			if ( Player.LocalInteractions[i] != None )
				...
				if (StreamInteraction(Player.LocalInteractions[i]) != None)

				s = StreamInteraction(Player.LocalInteractions[i]);
	}
	log("StreamInteraction is: "$s);
	return s;
}
...
//	function pawn.TakeDamage(int Damage, Pawn instigatedBy, Vector hitlocation, Vector momentum, class <damageType> damageType);
  exec function Fire(optional float F)
	{
	  if ((DeusExGameInfo(Level.Game).MapName=="00_Intro") || (GetFlagBase().GetBool('PlayerTraveling')))
	  return;
	  else
...
{
  log(self@"is destroyed! And NextURL is: "$Level.NextURL,'DeusExPlayerController');
  super.Destroyed();
}
...
{
//  RenameLockedFile();
  log("event NotifyLevelChange()",'DeusExPlayerController');
}

/*-- Набор функций для информации о сохранении --*/
/* Возвращает время с момента запуска игры */
/* Следует учитывать что Американская неделя начинается с воскресенья,
  в то время как наша - с понедельника. Помню в школе очень долго не могла
  к этому привыкнуть.
 */
