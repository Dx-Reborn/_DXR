//=============================================================================
// Soldier.
//=============================================================================
class Soldier extends HumanMilitary;

defaultproperties
{
     BindName="Soldier"
     FamiliarName="Soldier"
     UnfamiliarName="Soldier"
     CarcassType=Class'DeusEx.SoldierCarcass'
     WalkingSpeed=0.296000
     walkAnimMult=0.780000
     GroundSpeed=200.000000
     Mesh=mesh'DeusExCharacters.GM_Jumpsuit'
     skins(0)=Texture'DeusExCharacters.Skins.SoldierTex0'
     skins(1)=Texture'DeusExCharacters.Skins.SoldierTex2'
     skins(2)=Texture'DeusExCharacters.Skins.SoldierTex1'
     skins(3)=Texture'DeusExCharacters.Skins.SoldierTex0'
     skins(4)=Texture'DeusExItems.Skins.PinkMaskTex'
     skins(5)=Texture'DeusExItems.Skins.GrayMaskTex'
     skins(6)=Texture'DeusExCharacters.Skins.SoldierTex3'
     skins(7)=Texture'DeusExItems.Skins.PinkMaskTex'
     CollisionRadius=20.000000
     CollisionHeight=47.500000
}
