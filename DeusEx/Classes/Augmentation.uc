...
//	local Augmentation anAug;
	...
	if ( CurrentLevel < MaxLevel )
	{
		...
		augCan = AugmentationUpgradeCannisterInv(player.FindInventoryType(Class'AugmentationUpgradeCannisterInv'));
		if (augCan != None)
        bCanUpgrade = true;
	}
...
//	winInfo.SetTitle(AugmentationName);
	...
	if (bUsingMedbot)
	{
		winInfo.SetContent(Sprintf(OccupiesSlotLabel, AugLocsText[AugmentationLocation]));
