...
function FirstFrame()
{
	...
	if (localURL == "05_NYC_UNATCOMJ12LAB")
	{
		...
		if (!flags.GetBool('MS_InventoryRemoved'))
		{
			...
			if(DeusExWeaponInv(Player.inHand) != None)
				DeusExWeaponInv(Player.inHand).LaserOff();
			...
			if (Player.Inventory != None)
			{
				...
				foreach AllActors(class'SpawnPoint', SP, 'player_inv')
				{
					...
					while((item != None) && (item.IsA('NanoKeyRingInv') || (!item.bDisplayableInv)))
						item = item.Inventory;
					...
					if (item != None)
					{
						...
						if (item.IsA('ChargedPickupInv'))
							ChargedPickupInv(item).ChargedPickupEnd(Player);
						...
						if (item.IsA('DeusExWeaponInv') && (DeusExWeaponInv(item).AmmoType != None) && item.IsA('GrenadeWeaponInv')) // DXR: All grenades now inherited from GrenadeWeaponInv //!item.IsA('WeaponLAMInv') && !item.IsA('WeaponGasGrenadeInv') && !item.IsA('WeaponEMPGrenadeInv') && !item.IsA('WeaponNanoVirusGrenadeInv'))
							DeusExWeaponInv(item).PickupAmmoCount = DeusExWeaponInv(item).default.PickupAmmoCount;
					}
	...
	if (localURL == "05_NYC_UNATCOHQ")
	{
			...
			!flags.GetBool('MS_AnnaKilled'))
		{
			foreach AllActors(class'AnnaNavarre', Anna)
			{
				Anna.HealthTorso = 0;
				Anna.Health = 0;
				Anna.TakeDamage(1, Anna, Anna.Location, vect(0,0,0), class'DM_Shot');
			}
