...
//       if (GetPlayerPawn().AICanSee(testActor, 1.0, false, false, true, true) > 0.0)
       if (DeusExPlayer(Level.GetLocalPlayerController().pawn).AICanSee(testActor, 1.0, false, false, true, true) > 0.0)
           bCanSee = true;
    }
    else
    {
    ...
    if (PickRandomClass(classNum))
    {
        ...
        if (bSpawn)
        {
            ...
            if (i < ArrayCount(Pawns))
            {
                ...
                randRot = class'DeusExPawn'.static.RandomBiasedRotation(Rotation.Yaw, focus, Rotation.Pitch, focus);
                if (newPhysics == PHYS_Walking)
                    randRot.Pitch = 0;
                entryActor = Scout.PhysicsVolume.EntryActor;  // MAJOR hack!!!
                entrySound = Scout.PhysicsVolume.EntrySound;
                Scout.PhysicsVolume.EntryActor = None;
                Scout.PhysicsVolume.EntrySound = None;

                spawnee = Spawn(PawnClasses[classNum].PawnClass, self, , destination, randRot);
               // log(self@"Spawned creature "$spawnee);

                Scout.PhysicsVolume.EntryActor = entryActor;
                Scout.PhysicsVolume.EntrySound = entrySound;
                if (spawnee != None)
                {
