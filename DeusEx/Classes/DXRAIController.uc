...
/*    function HandleLoudNoise(Name event, EAIEventState state, XAIParams params) // пока нет менеджера событий.
    {
...
Begin:
    WaitForLanding();
    pawn.PlayWaiting();
    if ((pawn.Weapon != None) && ScriptedPawn(pawn).bKeepWeaponDrawn && (DeusExWeaponInv(pawn.Weapon).CockingSound != None) && !ScriptedPawn(pawn).bSeekPostCombat)
        pawn.PlaySound(DeusExWeaponInv(pawn.Weapon).CockingSound, SLOT_None,,, 1024);
    pawn.Acceleration = vect(0,0,0);
    if (!PickDestination())
        Goto('DoneSeek');
...
GoToLocation:
    ...
    if ((DeusExWeaponInv(pawn.Weapon) != None) && DeusExWeaponInv(pawn.Weapon).CanReload() && !pawn.Weapon.IsInState('Reload'))
        DeusExWeaponInv(pawn.Weapon).ReloadAmmo();
...
State Attacking
{
    ...
    function bool FireIfClearShot()
    {
        ...
        dxWeapon = DeusExWeaponInv(pawn.Weapon);
        if (dxWeapon != None)
        {
    ...
    function CheckAttack(bool bPlaySound)
    {
        ...
        if (ShouldFlee())
            ...
            bOutOfAmmo = True;
        else if (DeusexWeaponInv(pawn.Weapon).ReloadCount > 0)
        {
            if (DeusExWeaponInv(pawn.Weapon).AmmoType == None)
                bOutOfAmmo = True;
            else if (DeusExWeaponInv(pawn.Weapon).AmmoType.AmmoAmount < 1)
                bOutOfAmmo = True;
        }
    ...
    function bool IsHandToHand()
    {
        if (pawn.Weapon != None)
        {
            if (DeusExWeaponInv(pawn.Weapon) != None)
            {
                if (DeusExWeaponInv(pawn.Weapon).bHandToHand)
                    return true;
                else
                    return false;
            }
    ...
    function bool ReadyForWeapon()
    {
        ...
        if (DeusExWeaponInv(pawn.weapon) != None)
        {
            if (DeusExWeaponInv(pawn.weapon).bReadyToFire)
                if (!IsWeaponReloading())
                    bReady = true;
        }
        if (!bReady)
    ...
    function bool ShouldCrouch()
    {
        if (scriptedPawn(pawn).bCanCrouch && !pawn.PhysicsVolume.bWaterVolume && !IsHandToHand() &&
            ...
            ((DeusExWeaponInv(pawn.Weapon) == None) || DeusExWeaponInv(pawn.Weapon).bUseWhileCrouched))
            return true;
        else
            return false;
    }
