...
//  local InterpolateTrigger trig;
    ...
    {
//   flags.SetBool('Ray_dead',true,,17);// Used to test new scenes.
...
event Timer()
{
    ...
    if (localURL == "15_AREA51_BUNKER")
    {
                ...
                foreach AllActors(class'BlackHelicopter', chopper, 'heli_sabotaged')
                {
                    ...
                    for (i=0; i<20; i++)
                    {
                        ...
                        if (frag != None)
                        {
                            ...
                            frag.SetDrawScale(5.0 + 2.0 * FRand());
                            frag.Skins[0] = chopper.GetMeshTexture();
                            if (FRand() < 0.5)
                                frag.bSmoking = True;
                        }
                    }
        ...
        if (!flags.GetBool('MS_FanDestroyed'))
        {
            ...
            if (count == 0)
            {
                foreach AllActors(class'PhysicsVolume', zone, 'fan')
        ...
        if (flags.GetBool('MS_PageExploding'))
        {
            ...
            if ((Level.TimeSeconds - pageTimer >= 6.0) && !flags.GetBool('MS_PageDestroyed'))
            {
                foreach AllActors(class'DeusExMover', M, 'platform_pieces')
                    M.BlowItUp(Player);
                foreach AllActors(class'LifeSupportBase', Lbase)
                    Lbase.Destroy();
        ...
        for (i=0; i<ArrayCount(spawnData); i++)
        {
            if ((spawnData[i].count == 0) && (Level.TimeSeconds - spawnData[i].lastKilledTime > 20))
            {
                ...
                if (SP != None)
                {
                    ...
                    {
                        pawn.InitializePawn();
                        pawn.SetOrders('Patrolling', spawnData[i].orderTag, true); // Добавлено True (немедленно)
