...
function FirstFrame()
{
	...
	if (localURL == "00_TRAINING")
	{
		...
		foreach AllActors(class'UNATCOTroop', troop, 'Test_Subject')
		{
			...
			troop.Health = 0;
			troop.bStunned = True;
			troop.TakeDamage(1, troop, troop.Location, vect(0,0,0), class'DM_KnockedOut');
		}
	}
...
function Timer()
{
	...
	if (localURL == "00_TRAINING")
	{
		...
		// hide holo-Bob
		if (flags.GetBool('M00MeetPage_Played') && !flags.GetBool('MS_PageHidden'))
		{
...
function RemoveAllInventory()
{
	...
	if (Player.Inventory != None)
	{
		...
		do
		{
			if ((item != None) && item.bDisplayableInv || item.IsA('Ammunition'))
			{
   			...
   			if (item.IsA('ChargedPickupInv'))
					Player.RemoveChargedDisplay(ChargedPickupInv(item));
