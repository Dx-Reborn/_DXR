...
//var class name damageType;			// type of damage that this projectile does
...
var class<DeusExWeaponInv>	spawnWeaponClass;	// weapon to give the player if this projectile is disarmed and frobbed
...
function GrabProjectile(DeusExPlayer player)
{
	...
	if (player != None)
	{
		if (spawnWeaponClass != None)		// spawn the weapon
		{
			...
			if (item != None)
				DeusExWeaponInv(item).PickupAmmoCount = 1;
		}
