...
var() bool bNoCollision; // DXR: Turn off collision for actor 
...
function BeginPlay()
{
    ...
    for (i=0; i<numPathNodes-1; i++) {
        for (l=i+1; l<numPathNodes; l++) {
            if (Path[i].Position > Path[l].Position) {
...
function Trigger(actor Other, pawn EventInstigator)
{
    ...
    if( !bTriggeredOnce )
    {
        ...
        if (bNoCollision)
            PathActor.SetCollision(false, false, false);
...
// bStatic should be false by default (fixed)
defaultproperties
{
   bStatic=false
}
