...
function Sound GetDownSound()
{
    ...
    if (gl.bUseAltWeaponsSounds)
    {
        sound = class'DXRWeaponSoundManager'.static.GetFlameThrowerDown(gl.WS_Preset);
        if (sound != None)
        return sound;
        else
...
defaultproperties
{
     ...
     PickupViewMesh=Mesh'DeusExItems.FlamethrowerPickup'
     FirstPersonViewMesh=Mesh'DeusExItems.Flamethrower'
     Mesh=Mesh'DeusExItems.FlamethrowerPickup'
