    ...
    if (invItemTo != None)
    {
        ...
        else if ((invItemTo.IsA('DeusExWeapon')) && (DeusExPlayer(event.ToActor) != None))
        {
            ...
            if (AmmoType != None)
            {
                ...
                {
                    if (!AmmoType.AddAmmo(DeusExWeapon(invItemTo).PickUpAmmoCount))
                    {
                        invItemFrom.Destroy();
                        return nextAction;
                    }
...
/*  log("AddTransferCount()-------------------------------");
    ...
    if (invItemTo.IsA('DeusExWeapon'))
    {
        ...
        {
            AmmoType = ammunition(transferTo.FindInventoryType(DeusExWeapon(invItemTo).AmmoName));

            if ( AmmoType != None )
            {
                itemsTransferred = DeusExWeapon(invItemTo).PickUpAmmoCount * (event.transferCount - 1);
