...
var NetworkTerminalATM atmwindow;
...
function Frob(Actor Frobber, Inventory frobWith)
{
    ...
    if (Player != None)
    {
        if (bSuckedDryByHack)
        {
           pc.ClientOpenMenu("DeusEx.ATMDisabled");
           return;
        }
...
//          atmWindow = ATMWindow(root.InvokeUIScreen(Class'ATMWindow', True));
            atmwindow = NetworkTerminalATM(pc.OpenMenuEx("DeusEx.NetworkTerminalATM"));
            if (atmWindow != None)
            {
    ...
    if (bSync)
    {
        foreach AllActors(class'ATM', atm)
            for (i=0; i<atm.NumUsers(); i++)
                if (atm != Self)
                    if ((Caps(userList[userIndex].accountNumber) == atm.GetAccountNumber(i)) && (Caps(userList[userIndex].PIN) == atm.GetPIN(i)))
                    {
