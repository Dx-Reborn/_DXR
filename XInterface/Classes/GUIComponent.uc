...
//log("MenuStateChange:"$NewState@Self.Name);
    ...
    switch (MenuState)
    {
        ...
        case MSAT_Watched:
            PlayMouseOverSound(); // DXR: Play sound here
            if (bFocusOnWatch)
            {
                SetFocus(None);
                return;
...
function SetCursorPos(float newMouseX, float newMouseY)
{
    ...
    PC = PlayerOwner();
    PC.ConsoleCommand("SETMOUSE" @ newMouseX @ newMouseY);
}
...
function EnableWindow(bool bEnable)
{
   if (bEnable == true)
       EnableMe();
   else
...
//  NameSound = sound(DynamicLoadObject(SoundName, class'Sound'));
  if (bMouseOverSound)
      PlayerOwner().PlayOwnedSound(sound(DynamicLoadObject("DeusExSounds.UserInterface.Menu_Focus", class'Sound')),SLOT_Interface,0.25,true);
//      PlayerOwner().PlayOwnedSound(sound'DeusExSounds.UserInterface.Menu_Focus',SLOT_Interface,0.25,true);
} /*Controller.MouseOverSound*/
