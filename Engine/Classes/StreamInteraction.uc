...
//
// Изменено для корректного получения длительности звучания файла.
//
function SetStreamAttachment(Stream StreamObj)
{
	local StreamTag sTag;
	local string mm,ss, SL;
	...
	if ( CurrentStreamAttachment != None )
	{
		...
		if (sTag != None)
		{
			SL = sTag.Duration.FieldValue;
		  Divide(SL,":", mm,ss);
			CurrentSongDuration = float(mm$ss);
...
//
// Возврат длительности звучания файла
//
...
{ 
	if (!IsPlaying())
		...
		// Похоже что данный метод не работает
