...
#exec Texture Import File=Textures\S_Pickup.tga Name=S_Pickup Mips=Off MASKED=true ALPHA=true
#exec Texture Import File=Textures\S_NavP.tga Name=S_NavP Mips=Off MASKED=true ALPHA=true

// not used currently
#exec Texture Import File=Textures\S_Alarm.tga Name=S_Alarm Mips=Off MASKED=true ALPHA=true
...
var() editconst transient bool bCanReach;       // used during paths review in editor
...
var(Hidden) const bool bForceNoStrafing;// override any LD changes to bNeverUseStrafing
var const bool bAutoBuilt;      // placed during execution of "PATHS BUILD"
var(ExtraOptions) bool bSpecialMove;          // if true, pawn will call SuggestMovePreparation() when moving toward this node
var(ExtraOptions) bool bNoAutoConnect;      // don't connect this path to others except with special conditions (used by LiftCenter, for example)
...
var(ExtraOptions)   bool    bForcedOnly;            // only connect forced paths to this NavigationPoint

var(ReachSpec) editconst const array<ReachSpec> PathList; //index of reachspecs (used by C++ Navigation code)
...
var editconst vector MaxPathSize;
