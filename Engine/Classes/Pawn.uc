...
#exec Texture Import File=Textures\S_Pawn.tga Name=S_Pawn Mips=Off MASKED=true ALPHA=true
...
simulated function DisplayDebug(Canvas Canvas, out float YL, out float YPos)
{
    ...
    Canvas.DrawText("Animation Action "$AnimAction$"ˇ Health = "$Health);
    YPos += YL;
    ...
    YPos += YL;
    Canvas.SetPos(4,YPos);
    Canvas.DrawText("EyeHeight "$Eyeheight$" BaseEyeHeight "$BaseEyeHeight$ " Visibility ="$Visibility);//" Physics Anim "$bPhysicsAnimUpdate);
    YPos += YL;
    Canvas.SetPos(4,YPos);
    ...
    else
    {
        ...
        {
            Canvas.SetDrawColor(255,0,0);
            Canvas.DrawText("Owned by "$Controller.PlayerReplicationInfo.PlayerName); // ƒÓ·‡‚ÎÂÌÓ
            YPos += YL;
            Canvas.SetPos(4,YPos);
        }
...
simulated function rotator AdjustAim(float projSpeed, vector projStart, int aimerror, bool leadTarget, bool warnTarget)
{
//  if ( Controller == None )
        return GetViewRotation(); //Rotation;
  // ›“Œ »ƒ»Œ“»«Ã!!
//  return Controller.AdjustAim(FiredAmmunition, projStart, aimerror);
  //.function rotator AdjustAim(projSpeed, projStart, aimerror, leadTarget, warnTarget); //AdjustAim(FiredAmmunition, projStart, aimerror);
}
