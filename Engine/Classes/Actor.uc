...
#exec Texture Import File=Textures\S_Actor.tga Name=S_Actor Mips=Off MASKED=true ALPHA=true
...
var(Relative) const vector        RelativeLocation;   // location relative to base/bone (valid if base exists)
var(Relative) const rotator       RelativeRotation;   // rotation relative to base/bone (valid if base exists)
...
const MAXSTEPHEIGHT = 25.0; // Maximum step height walkable by pawns// DXR: Patched to 25, changed here too.
...
// ѕоскольку дополнительные переменные € добавить не могу (происходит вылет),
// € добавлю, функции, возвращающие то, что мне нужно.
function string GetBindName();
function string GetBarkBindName(); // Used to bind Barks!

// Ќу, какие еще запросы?
function float GetBaseEyeHeight();  // Base eye height above collision center.
function float GetEyeHeight();      // Current eye height, adjusted for bobbing and stairs.
...
function bool GetbInCon(); // For ConvoHandler
function SetbInCon(optional bool NewValue); // For ConvoHandler

function array<object> GetConListItems(); // возвращает conListItems
function SetConListItems(object AconListItems); // выставл€ет conListItems
function object NoneConListItems(); // выставл€ет conListItems в none
...
final static function string Sprintf(string fmt,optional coerce string s0,optional coerce string s1,optional coerce string s2,optional coerce string s3)
{
...
// ----------------------------------------------------------------------
// From Extension/Window.uc (DeusEx 1.112fm)
// ----------------------------------------------------------------------
...
// ƒл€ удобства (из нулевого канала)
...
simulated function float GetAnimRate()
{
    local name anim;
    local float frame, rate;
