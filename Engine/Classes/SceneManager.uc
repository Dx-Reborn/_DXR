...
#exec Texture Import File=Textures\S_SceneManager.tga  Name=S_SceneManager Mips=Off

// Graphics for UI
#exec Texture Import File=Textures\S_MatineeIP.tga Name=S_MatineeIP Mips=Off MASKED=true ALPHA=true
#exec Texture Import File=Textures\S_MatineeIPSel.tga Name=S_MatineeIPSel Mips=Off MASKED=true ALPHA=true
#exec Texture Import File=Textures\S_MatineeTimeMarker.tga Name=S_MatineeTimeMarker Mips=Off MASKED=true ALPHA=true
#exec Texture Import File=Textures\S_ActionCamMove.tga  Name=S_ActionCamMove Mips=Off ALPHA=true

#exec Texture Import File=Textures\S_ActionCamPause.tga  Name=S_ActionCamPause Mips=Off ALPHA=true
#exec Texture Import File=Textures\S_PathLinear.tga  Name=S_PathLinear Mips=Off MASKED=true ALPHA=true
#exec Texture Import File=Textures\S_PathBezier.tga  Name=S_PathBezier Mips=Off MASKED=true ALPHA=true
#exec Texture Import File=Textures\S_BezierHandle.tga  Name=S_BezierHandle Mips=Off MASKED=true ALPHA=true
#exec Texture Import File=Textures\SubActionIndicator.tga  Name=SubActionIndicator Mips=Off MASKED=true ALPHA=true
...
// DXR: removed transient, so now it will work correctly after loading saved game.
...
event SceneStarted()	// Called from C++ when the scene starts.
{
	...
	Viewer = None;
	if /*(*/(Affect==AFFECT_Actor) //&& (Viewer != none)) // It can be 'None' now !
	...
	else
	{
		for( P = Level.ControllerList ; P != None ; P = P.nextController )
			if( P.IsA('PlayerController')  )
			{
				...
				PlayerController(Viewer).MyHud.HideHUD();// bHideHUD = bHideHUD;
				break;
			}
	}
...
event SceneEnded()		// Called from C++ when the scene ends.
{
	...
	if( Affect==AFFECT_ViewportCamera )
	{
		if ( PlayerController(Viewer) != None )
		{
			...
			PlayerController(Viewer).MyHud.ShowHUD();//bHideHUD = false;
        }
	}
