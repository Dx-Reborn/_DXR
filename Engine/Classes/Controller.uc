...
function DisplayDebug(Canvas Canvas, out float YL, out float YPos)
{
	...
	if ( Enemy != None )
		Canvas.DrawText("     STATE: c:ë"$GetStateName()$"ï Timer: "$TimerCounter$" Enemy "$Enemy/*.GetHumanReadableName()*/, false);
	else
		Canvas.DrawText("     STATE: c:ë"$GetStateName()$"ï Timer: "$TimerCounter$" NO Enemy ", false);
	YPos += YL;
	Canvas.SetPos(4,YPos);
	...
	if ( PlayerReplicationInfo == None )
		Canvas.DrawText("     NO PLAYERREPLICATIONINFO" $"jjÿ bAdvancedTactics = "$bAdvancedTactics, false);
	else
